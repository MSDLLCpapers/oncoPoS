...
## Indication-specific surrogate-primary endpoint relationships (e.g., ORR-> PFS) \label{sec:indication_specific}
...
$\theta^P_j$: treatment effect on endpoint $P$ at trial $j$. $\sigma^P_j$: standard error of treatment effect on endpoint $P$ at trial $j$. 

\begin{itemize}
  \item \textbf{Level 1 observed trial-level model:}\\
  Each trial reports an observed log-odds ratio for ORR, $\hat\theta^{\text{ORR}}_j$, 
  and an observed log-hazard ratio for PFS, $\hat\theta^{\text{PFS}}_j$, and we model them as:
  \[
  \hat\theta^{\text{ORR}}_j \sim \mathcal{N}\bigl(\theta^{\text{ORR}}_j,   \ (\sigma^{\text{ORR}}_j)^2\bigr)
  \]
  \[
  \hat\theta^{\text{PFS}}_j \sim \mathcal{N}\bigl(\theta^{\text{PFS}}_j,   \ (\sigma^{\text{PFS}}_j)^2\bigr)
  \]
  Conditional on the (latent) true PFS effect, the true ORR effect follows an indication-specific regression:
  \[
    \theta^{\text{ORR}}_j
      \;\sim\;
      \mathcal{N}\!\Bigl(\alpha_{z_j} + \beta_{z_j}\,\theta^{\text{PFS}}_j,\;
      \sigma_{\text{WLS}}^{2}/N_{\text{patients},j}\Bigr).
  \]

  \item \textbf{Level 2 indication-specific regression parameters:}\\
  Share information across indications by hierarchical modeling. Each pair is learned adaptively across indications.
  \[
  \alpha_k \sim \mathcal{N}(a_0,\ s_0^2), \quad
  \beta_k \sim \mathcal{N}(b_0,\ s_1^2), \quad
  k = 1,\dots,5
  \]
  \item \textbf{Level 3 population level (hyperpriors):}
\[
